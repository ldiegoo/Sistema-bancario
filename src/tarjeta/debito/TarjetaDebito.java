...
public class TarjetaDebito extends Tarjeta implements Serializable {
    ...
    // Constructor
    public TarjetaDebito(String titular, String numTarjeta, LocalDate fechaCreacion, String CVV, String CLABE, LocalDate fechaVencimiento,double saldo) {
    ...
    public String retirarDinero(double monto) {
        try {
            if (monto <= 0) {
                ...
                saldo -= monto;
                return "El retiro se ha realizado con éxito. Su saldo disponible ahora es de : $" + String.format("%.2f", saldo);
            }
        } catch (IllegalArgumentException e) {
            return "Error: " + e.getMessage();
        } catch (Exception e) {
